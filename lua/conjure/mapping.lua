...
_2amodule_2a["on-quit"] = on_quit0
do local _ = {on_quit0, nil} end
local function init0(filetypes)
  nvim0.ex.augroup("conjure_init_filetypes")
  nvim0.ex.autocmd_()
  if (true == config0["get-in"]({"mapping", "enable_ft_mappings"})) then
    nvim0.ex.autocmd("FileType", str0.join(",", filetypes), bridge0["viml->lua"]("conjure.mapping", "on-filetype", {}))
  else
  end
...
local function connect_command0(...)
  local args = {...}
  local function _15_(...)
    if (1 == a0.count(args)) then
      local host, port = string.match(a0.first(args), "([a-zA-Z%d\\.-]+):(%d+)$")
  ...
  return client0.call("connect", _15_(...))
end
...
local function omnifunc0(find_start_3f, base)
  if find_start_3f then
    local _let_17_ = nvim0.win_get_cursor(0)
    local row = _let_17_[1]
    local col = _let_17_[2]
    local _let_18_ = nvim0.buf_get_lines(0, a0.dec(row), row, false)
    local line = _let_18_[1]
    return (col - a0.count(nvim0.fn.matchstr(string.sub(line, 1, col), "\\k\\+$")))
  else
    return eval0["completions-sync"](base)
  end
end
_2amodule_2a["omnifunc"] = omnifunc0
do local _ = {omnifunc0, nil} end
nvim0.ex.function_(str0.join("\n", {"ConjureEvalMotionOpFunc(kind)", "call luaeval(\"require('conjure.eval')['selection'](_A)\", a:kind)", "endfunction"}))
...
local function _20_(_241)
  return eval_ranged_command0((_241).line1, (_241).line2, (_241).args)
end
nvim0.create_user_command("ConjureEval", _20_, {nargs = "?", range = true})
local function _21_(_241)
  return connect_command0(unpack((_241).fargs))
end
nvim0.create_user_command("ConjureConnect", _21_, {nargs = "*", range = true, complete = "file"})
local function _22_(_241)
  return client_state_command0((_241).args)
end
nvim0.create_user_command("ConjureClientState", _22_, {nargs = "?"})
local function _23_()
  return school0.start()
end
nvim0.create_user_command("ConjureSchool", _23_, {})
return _2amodule_2a
