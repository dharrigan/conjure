...
_2amodule_2a["config-dir"] = config_dir0
do local _ = {config_dir0, nil} end
local function findfile0(name, path)
  local res = nvim0.fn.findfile(name, path)
  if not a0["empty?"](res) then
    return res
  else
    return nil
  end
...
_2amodule_2a["localise-path"] = localise_path0
do local _ = {localise_path0, nil} end
local function file_path__3emodule_name0(file_path)
  if file_path then
    local function _14_(mod_name)
      local mod_path = string.gsub(mod_name, "%.", afs0["path-sep"])
      if (text0["ends-with"](file_path, (mod_path .. ".fnl")) or text0["ends-with"](file_path, (mod_path .. "/init.fnl"))) then
        return mod_name
      else
        return nil
      end
    end
    return a0.some(_14_, a0.keys(package.loaded))
  else
    return nil
  end
end
